class DefaultDict {
  constructor(type) {
    ...
    if (type !== "int" && type !== "list" && type !== "set")
      throw Error("타입을 확인해주세요");
    this.type = type;
    this.dict = new Map();
  }
  ...
  get(value) {
    if (!this.dict.has(value)) {
      ...
      else if (this.type === "list") return [];
      // 여기서 push를 하면 반영이 안되는데, 조심하는 수 밖에 없을까?
      else return new Set();
  ...
  set(key, value) {
    if (this.type === "int") {
      ...
      this.dict.set(
        key,
        isExist ? this.dict.get(key).add(value) : new Set([value])
      );
    }
  }
}
